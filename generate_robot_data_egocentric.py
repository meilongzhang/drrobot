...
class MujocoActor:
    ...
    def _get_hand_lookat(self, joint_position):
        ...
        primary_tokens = ("hand", "palm", "gripper", "wrist")
        secondary_tokens = ("ee", "end_effector")
        ...
        for body_id in range(self.model.nbody):
            ...
            if any(token in name for token in primary_tokens):
                generic_candidates.append(body_id)
                if "left" in name or name.startswith("l_") or name.startswith("lh_"):
                    left_candidates.append(body_id)
                if "right" in name or name.startswith("r_") or name.startswith("rh_"):
                    right_candidates.append(body_id)
            elif any(token in name for token in secondary_tokens):
                generic_candidates.append(body_id)
                if "left" in name:
                    left_candidates.append(body_id)
                if "right" in name:
        ...
        if left_candidates and right_candidates:
            left_pos = self.data.xpos[left_candidates[-1]].copy()
            right_pos = self.data.xpos[right_candidates[-1]].copy()
            return 0.5 * (left_pos + right_pos)

        if generic_candidates:
            hand_positions = self.data.xpos[generic_candidates].copy()
    ...
    def generate_and_save_pc(self, sample_id, args, is_canonical=False, is_test=False, verbose=False):
        ...
        lookat = self._get_hand_lookat(joint_position)
