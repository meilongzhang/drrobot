...
def apply_camera_rotation_offset(
    w2c: torch.Tensor,
    pitch_deg: float = 0.0,
    yaw_deg: float = 0.0,
    roll_deg: float = 0.0,
    ...
    """
    Apply an extra local-camera rotation after loading extrinsics.

    Angles are in degrees and use the camera's local axes:
    - pitch: rotate around camera +X
    - yaw: rotate around camera +Y
    - roll: rotate around camera +Z
    """
    w2c_np = w2c.detach().cpu().numpy()

    delta_cam = Rotation.from_euler(
        "xyz",
    ...
    T_delta[:3, :3] = delta_cam

    rotated_w2c = T_delta @ w2c_np
    return torch.tensor(rotated_w2c, dtype=torch.float32, device=w2c.device)


def apply_camera_translation_offset(
    w2c: torch.Tensor,
    tx: float = 0.0,
    ty: float = 0.0,
    tz: float = 0.0,
    ...
    """
    Apply an extra local-camera translation after loading extrinsics.

    Offsets are expressed in the camera frame:
    - tx: shift along camera +X
    - ty: shift along camera +Y
    - tz: shift along camera +Z
    """
    w2c_np = w2c.detach().cpu().numpy()

    T_delta = np.eye(4, dtype=np.float32)
    T_delta[:3, 3] = np.array([tx, ty, tz], dtype=np.float32)

    translated_w2c = T_delta @ w2c_np
    return torch.tensor(translated_w2c, dtype=torch.float32, device=w2c.device)
...
def main():
    ...
    extra_ty = 0.0
    extra_tz = 0.2
    w2c = apply_camera_rotation_offset(
        w2c,
        pitch_deg=extra_pitch_deg,
        yaw_deg=extra_yaw_deg,
        roll_deg=extra_roll_deg,
    )
    w2c = apply_camera_translation_offset(
        w2c,
        tx=extra_tx,
        ty=extra_ty,
        tz=extra_tz,
