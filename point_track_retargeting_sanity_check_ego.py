...
def _parse_vec(attr_value: str, default):
    if attr_value is None:
        return np.asarray(default, dtype=np.float32)
    return np.fromstring(attr_value, sep=" ", dtype=np.float32)


def _quat_to_rotmat(quat_wxyz: np.ndarray):
    quat_wxyz = np.asarray(quat_wxyz, dtype=np.float32)
    quat_wxyz = quat_wxyz / max(np.linalg.norm(quat_wxyz), 1e-8)
    w, x, y, z = quat_wxyz
    return np.array(
        [
...
def load_robot_body_transforms(robot_xml_path: str):
    tree = ET.parse(robot_xml_path)
    root = tree.getroot()
    worldbody = root.find("worldbody")
    if worldbody is None:
        raise ValueError(f"Could not find <worldbody> in {robot_xml_path}")

    body_positions = {}
    body_rotations = {}

    def visit_body(body_elem, parent_pos, parent_rot):
        body_name = body_elem.get("name")
        local_pos = _parse_vec(body_elem.get("pos"), [0.0, 0.0, 0.0])
        local_quat = _parse_vec(body_elem.get("quat"), [1.0, 0.0, 0.0, 0.0])
        local_rot = _quat_to_rotmat(local_quat)

        world_pos = parent_pos + parent_rot @ local_pos
        world_rot = parent_rot @ local_rot

        if body_name:
            body_positions[body_name] = world_pos
            body_rotations[body_name] = world_rot

        for child_body in body_elem.findall("body"):
            visit_body(child_body, world_pos, world_rot)

    identity = np.eye(3, dtype=np.float32)
    for body_elem in worldbody.findall("body"):
        visit_body(body_elem, np.zeros(3, dtype=np.float32), identity)

    return body_positions, body_rotations


def build_left_hand_egocentric_w2c(
    robot_xml_path: str,
    hand_body_name: str = "lh_palm",
    hand_forward_axis = np.array([0.0, 0.0, 1.0], dtype=np.float32),
    hand_right_axis = np.array([1.0, 0.0, 0.0], dtype=np.float32),
    head_right_offset: float = 0.08,
    head_up_offset: float = 0.20,
    head_back_offset: float = 0.10,
    look_down_offset: float = 0.05,
):
    body_positions, body_rotations = load_robot_body_transforms(robot_xml_path)
    if hand_body_name not in body_positions:
        raise ValueError(f"Could not find hand body '{hand_body_name}' in {robot_xml_path}")

    hand_position = body_positions[hand_body_name]
    hand_rotation = body_rotations[hand_body_name]

    hand_forward_axis = hand_forward_axis / max(np.linalg.norm(hand_forward_axis), 1e-8)
    hand_right_axis = hand_right_axis / max(np.linalg.norm(hand_right_axis), 1e-8)
    world_up = np.array([0.0, 0.0, 1.0], dtype=np.float32)

    forward_dir = hand_rotation @ hand_forward_axis
    right_dir = hand_rotation @ hand_right_axis

    # Define "higher" in robot/world frame, and project hand directions to the horizontal plane.
    forward_dir = forward_dir - np.dot(forward_dir, world_up) * world_up
    if np.linalg.norm(forward_dir) < 1e-8:
        forward_dir = np.array([1.0, 0.0, 0.0], dtype=np.float32)
    forward_dir = forward_dir / max(np.linalg.norm(forward_dir), 1e-8)

    right_dir = right_dir - np.dot(right_dir, world_up) * world_up
    if np.linalg.norm(right_dir) < 1e-8:
        right_dir = np.array([0.0, -1.0, 0.0], dtype=np.float32)
    right_dir = right_dir / max(np.linalg.norm(right_dir), 1e-8)

    camera_position = hand_position + (
        head_right_offset * right_dir
        + head_up_offset * world_up
        - head_back_offset * forward_dir
    )

    look_target = hand_position - look_down_offset * world_up
    forward = look_target - camera_position
    ...
    rotation_w2c = np.stack([right, down, forward], axis=0)
    translation_w2c = -rotation_w2c @ camera_position

    w2c = np.eye(4, dtype=np.float32)
    w2c[:3, :3] = rotation_w2c
    w2c[:3, 3] = translation_w2c
...
def main():
    ...
    robot_xml_path = os.path.join(model_path, "robot_xml", "left_hand.xml")
    ...
    w2c = build_left_hand_egocentric_w2c(
        robot_xml_path=robot_xml_path,
        hand_body_name="lh_palm",
        head_right_offset=0.08,
        head_up_offset=0.20,
        head_back_offset=0.10,
        look_down_offset=0.05,
    )
